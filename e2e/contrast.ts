 ...
 *    files the node under `incomplete`. That is most of this page: twenty-one
 *    `color-mix(in oklab, <token> N%, transparent)` washes carry the badges,
 *    the prediction boxes, the hero hook, the `.reduce-current` step
 *    highlight, the LWE/SIS row tints and the number-line bands, and every one
 *    of them composites onto whatever card is behind it rather than onto a
 ...
 *    `opacity: .85`, and `#app button:disabled` at `.55` — the state every
 *    answered exit-check option and every settled prediction lands in.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. Every numeric
 *    table on this page sits in `.table-scroll { overflow-x: auto }`, the
 *    reduction stepper's basis lives in `.reduce-matrix { overflow-x: auto }`
 *    whose rows are `white-space: nowrap`, and `#app details` carries
 *    `overflow-x: clip`. At 380px all three clip. Content scrolled past the
 *    client box is not dimmed or partly drawn — it is absent from the frame,
 ...
 *    rely on the wider viewport where the very same element is visible and
 *    measured for real.
 *
 ...
 *    load-bearing here, not defensive: the lattice views and the toy-Kyber
 *    number line both draw their shapes first and their `<text class="lv-label">`
 *    afterwards, so a label's backdrop is a PRECEDING SIBLING `<rect>` — the
 *    `.band-zero` / `.band-one` decode bands — not an ancestor. An
 ...
 *    composited first.
 *
 * The gradient machinery below is kept although this page authors no
 * `linear-`/`radial-gradient` at all: `paintAt` calls it for every element and
 * it returns transparent for `background-image: none`, so it costs nothing and
 * is correct the day a gradient lands.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: every surface tint on
     * this page is `color-mix(in oklab, ...)` — twenty-one of them — and
     ...
     *
     * An earlier version of this helper judged every gradient at its worst
     * *stop*, assuming that stop covered the text wherever the text sat. That
     * is right for a gradient spanning its element and wrong for a decorative
     * corner glow on a page many viewports tall: on one lab it invented
     * backdrops the footer text never sits on (accent links read 3.56:1 where
     ...
     *
     * In reference mode this page mounts all five exhibits at once: two lattice
     * SVGs, a 5x3 and a 3x5 modular matrix, the LWE comparison table, the
     * reduction log, four polynomial vectors and the exit check's twenty option
     * buttons — and every one of them walks the same handful of ancestors.
     * Without
     ...
     *
     * ONLY ELEMENTS THAT ACTUALLY PAINT A FILL COUNT. `fill` is an inherited
     * property whose initial value is opaque BLACK, and `getComputedStyle`
     * happily reports it for elements that never paint one: a `<g>` is a
     * container, and `<line>`/`<polyline>` have no fill region at all. Their
     * bounding boxes are large — a `<g>` wrapping every lattice dot spans the
     * whole diagram — so a walk that trusts `fill` composites an opaque black
     * rectangle under any label that falls inside one, and then reports a ratio
     * against a surface nothing on screen has. Measured on this lab: the CVP
     * target label `t` came back at 2.48:1 against "rgb(0, 0, 0)" in the light
     * theme, where it actually renders `#6d4400` on `--bg-inset` #eaf1f7 —
     * 7.44:1. It was silent in the dark theme, where the same fabricated black
     * happened to clear the threshold, which is how it survived.
     */
    const PAINTS_FILL = new Set(['rect', 'circle', 'ellipse', 'polygon', 'path']);
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      for (const s of stack.reverse()) {
        if (!PAINTS_FILL.has(s.tagName.toLowerCase())) continue;
     ...
     * only a ZERO-area clip qualifies, so a real partial clip is still
     * measured.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has ten disclosures — the "Stuck? Reveal the
      // known solutions" pair, the two "Where is the lattice…" formulations,
      // the toy-vs-standard delta tables, the KEM layer, and the two "Inspect
      // the secrets" teaching views — every one of which ships CLOSED and is
      // opened by the gate with a real click.
     ...
     * catch what axe *misses* among exposed text (translucency, opacity), not
     * to be stricter than axe on decorative content. Honour the same boundary:
     * every `badge()` on this page prefixes its label with an `aria-hidden`
     * icon span (✓ / ⚠ / ✗ / ℹ) whose visible twin is the badge text beside it,
     * already measured.
     */
     ...
     * against the panel. This lab builds its SVG with `createElementNS` and
     * trips none of it; the guard is cheap and silent when unused.
     */
     ...
     * this — the walk breaks out as soon as the backdrop is opaque, which on
     * this page is `.card`'s `--bg-panel` for most text. What does reach it is
     * everything sitting directly on the page: the hero, the guide rail, the
     * mode hint and the footer.
     */
